...
import "./styles/AddNewItem.sass"; // импорт стилей

const AddNewItem = () => {
	const history = useHistory();
	const db = getFirestore();
	const location = useLocation();
	const { appStateFields } = location.state; // содержимое анкеты
	const initialState = {
		content: {
			name: "",
			...
			data: [
				{
					...
					fields: ["ИНН", "Юридическое название", "Юридический адрес"],
				},
				{
					title: "Документы",
					fields: ["Свидетельство о государственной регистрации юридического лица"],
				},
			],
		},
		template: false,
	...
	const saveChanges = () => {
		...
		setDoc(asignRef, { fields: [...appStateFields, addFormState] });
		history.push("/");
	};
	const addBlock = () => {
		setAddFormState({
